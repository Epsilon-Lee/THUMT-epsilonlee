...
def decoding_graph(features, state, mode, params):
    ...
    if params.right2left:
        tgt_seq = tf.reverse_sequence(tgt_seq, tgt_len, seq_dim=1)
    src_mask = tf.sequence_mask(src_len,
                                maxlen=tf.shape(features["source"])[1],
                                dtype=tf.float32)
    ...
    enc_attn_bias = layers.attention.attention_bias(src_mask, "masking")
    dec_attn_bias = layers.attention.attention_bias(tf.shape(decoder_input)[1],
                                                    "causal")
    # Shift left
    ...
    ce = tf.reshape(ce, tf.shape(tgt_seq[:, 1:]))

    if mode == "eval":
        return -tf.reduce_sum(ce * tgt_mask[:, 1:], axis=1)

    loss = tf.reduce_sum(ce * tgt_mask[:, 1:]) / tf.reduce_sum(tgt_mask[:, 1:])
